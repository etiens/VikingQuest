...
//	public Ocean ocean;
	...
	public Collider buoyantCollider;
	
	public GameObject target;
	
	public float TotalForce {get; private set;}
	...
	private List<Vector3> SliceIntoVoxels(bool concave)
	{
		...
		{
			// Convex slicing
			var bounds = buoyantCollider.bounds;//GetComponent<Collider>().bounds;
			for (int ix = 0; ix < slicesPerAxis; ix++)
			{
				for (int iy = 0; iy < slicesPerAxis; iy++)
				{
...
//		return ocean == null ? 0.0f : ocean.GetWaterHeightAtLocation(x, z);
	...
	private void FixedUpdate()
	{
		...
		foreach (var point in voxels)
		{
			...
			if (wp.y - voxelHalfHeight < waterLevel)
			{
				...
				var velocity = target.rigidbody.GetPointVelocity(wp);
				var localDampingForce = -velocity * DAMPFER * target.rigidbody.mass;
				var f = Mathf.Sqrt(k);
				var force = /*localDampingForce + */f * localArchimedesForce;
				target.rigidbody.AddForceAtPosition(force, wp);
	...
	private void OnDrawGizmos()
	{
		...
		foreach (var force in forces)
		{
			...
			Gizmos.DrawLine(force[0], force[0] + force[1] / target.rigidbody.mass);
		}
	}
}